...
--[[
	...
	* *使用描边效果后，ttfConfig.distanceFieldEnabled=false，否则没有效果*
	...
	+ underline = 2, #ff0099  - (underline_size, underline_color)
	...
	**示例：**
	...
	local test_text = {
	    ...
	    "hello<pos x=1 y=2 scale=0.5 rotate=90 visible=true />world",
	}
...
function RichLabel:ctor(params)
	params = params or {}
	local fontName 	= params.fontName or NORMALFONT
	...
	self._leftSpaceWidth = maxWidth
	self._isAutoAdjustWidth = (maxWidth <= 0)
...
function RichLabel:setAnchorPoint(anchor, anchor_y)
	...
	if self._currentText then
		...
		local origin_x, origin_y = 0, self._currentHeight
		local result_x = origin_x - anchor.x * self._currentWidth
		local result_y = origin_y - anchor.y * self._currentHeight
		self._containerNode:setPosition(result_x, result_y)
	end
end
...
function RichLabel:setLabelParser(labelname, parser)
	if type(parser) == "function" then
		self.tempParsers[labelname] = parser
	end
end
...
function RichLabel:charsToNodes_(parsedtable, containerNode)
	...
	for index, params in pairs(parsedtable) do
		...
		-- 检测是否存在解析器
		local parser
		if self.tempParsers[labelname] then
			parser = self.tempParsers[labelname]
		else
			parser = self:loadLabelParser_(labelname)
		end
		if not parser then
...
function RichLabel:adjustLineBreak_(allnodelist, charspace)
	...
	if self._isAutoAdjustWidth then maxwidth = 999999999999
	end
	-- 存放每一行的nodes
	local alllines = {{}, {}, {}}
	-- 当前行的累加的宽度
...
function RichLabel:addNewLine()
	self._leftSpaceWidth = self._maxWidth
end
...
--获取字符串UTF8长度
function RichLabel:getCharacterCountInUTF8String(str)
	local len = string.len(str)
    local i = 1 
    local number = 0
    while i <= len do
...
--截取UTF8字符串
function RichLabel:getSubStringOfUTF8String(str, startIndex, length)
	local len = string.len(str)
    local i = startIndex 
    local number = 0
    local subStr = ""
    while i <= len do
    	--print("getSubStringOfUTF8String ", str,  i)
        local c = string.byte(str, i)
        local shift = 1
        if c > 0 and c <= 127 then
            shift = 1
    ...
    subStr = string.sub(str, startIndex, i-1) or subStr
...
function RichLabel:getSpine(path)
    ...
    sp.SkeletonCache:getInstance():loadSkeletonData(path, jsonPath, plistPath, 1)
    local spine = sp.SkeletonAnimation:createWithKey(path, scale)

    table.insert(self.spineList, spine)

    return spine
end

function RichLabel:getPos(pos, imgPath, useClick, outline)
	if not pos then
        return
    end
    ...
    if useClick then
    	onClick = function (me)
			...
			openPosWin({pos = pos, cName = "1"})
    	end
    end

    local UIImageBox = OpenFile("UIImageBox")
    local imgPath = imgPath or "public/public_button_031.png"
    local box = UIImageBox.new(imgPath, onClick, {textParams = {text = posString(pos), size = 22}})
    
    if outline then--带下划线坐标
    	box:setText(string.format("[%s,%s]", pos.x, pos.y))
    	local btnLabel = box.label
		box:setScale9Enabled(true)
    	box:setContentSize(btnLabel:getContentSize())

    	local boxSize = box:getContentSize()
    	btnLabel:setPosition(boxSize.width * 0.5, boxSize.height * 0.5)
    	btnLabel:setTextColor(cc.c3b(238,159,0))
   	end

    return box
end

function RichLabel:doUpdate(ticks)
	for k,v in pairs(self.spineList) do
		v:update(ticks)
	end
end

function RichLabel:autoUpdate()
	for k,v in pairs(self.spineList) do
		v:scheduleUpdate(function()end)
	end
end
