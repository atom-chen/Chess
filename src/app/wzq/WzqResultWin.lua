...
local WzqResultWin = class("WzqResultWin", WinBase)

function WzqResultWin:ctor(aiName, win, total)
    WzqResultWin.super.ctor(self)
    self.aiName = aiName or ""
    self.win = win
    self.total = total or 0
end

function WzqResultWin:onWinEnter()
    self.backFrame = cc.CSLoader:createNode("wzq/result.csb")
